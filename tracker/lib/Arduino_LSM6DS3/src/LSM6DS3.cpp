...
int LSM6DS3Class::begin(bool highSampleRate)
{
  if (_spi != NULL) {
  ...
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // Changes to allow for a 208Hz sample rate
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  if (!highSampleRate) { // 104 Hz operation
  ...
  } else { // 208 Hz operation
    //set the gyroscope control register to work at 208 Hz, 2000 dps and in bypass mode
    // 01011100
    writeRegister(LSM6DS3_CTRL2_G, 0x5C);
    // Set the Accelerometer control register to work at 208 Hz, 4 g,and in bypass mode and enable ODR/4
    // low pass filter (check figure9 of LSM6DS3's datasheet)
    // 01011001
    writeRegister(LSM6DS3_CTRL1_XL, 0x59);
  }
